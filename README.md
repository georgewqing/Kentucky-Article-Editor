...
- **Dialogue / 台词:** Chat-style `*.dialogue.csv` + root `characters.csv` (`model_node`); file meta `*.dialogue.meta.json` (`godot_scene`, `dialogue_id`); auto filename from scene + dialogue id; rename via explorer. Godot same-folder hot-edit contract: [`extras/godot-kentucky-dialogue/README.md`](./extras/godot-kentucky-dialogue/README.md). / 聊天式台词；meta + 节点名；文件名自动生成；Godot 契约见 extras。
