# OPEN: Workbench Chrome 对齐（选区菜单 · 挂载 · Skill 胶囊 · 文件夹展开记忆）

> **状态：OPEN / 未开始或仅部分预同步 CSS**  
> **创建**：2026-08-11  
> **Win changelog**：[`../../win/project-memory/changelog.md`](../../win/project-memory/changelog.md) **§70–73**  
> **Win 会话交接**：[`../../win/project-memory/SESSION-TOOL-FEEDBACK.md`](../../win/project-memory/SESSION-TOOL-FEEDBACK.md)  
> **Agent `/` + 上下文用量**：[`OPEN-agent-ui-parity.md`](./OPEN-agent-ui-parity.md)（U1–U3；本文 U4–U7 互补）  
> **进度板**：[`OPEN-agent-tool-feedback-parity.md`](./OPEN-agent-tool-feedback-parity.md)  
> **移植手册**：[`PORTING-WIN-TO-ANDROID.md`](./PORTING-WIN-TO-ANDROID.md) §阶段 G · 第 12 条  
> **勿 import `win/`**

Win 本轮在工作台 / Agent 面板落地的 **chrome 交互** 汇总。Android 按章节移植；CSS 部分类名可能已先同步，**组件与 runtime 契约仍以本文为准**。
...
| ID | 项 | Win | Android | 说明 |
|----|-----|-----|---------|------|
| U4 | Composer / 气泡文件挂载 chip + 拖入 | ✅ | ❌ CSS 部分 | §2；无缩略图 |
| U5 | Skill 暖色胶囊 + 发送注入 skill 正文 | ✅ | ❌ | §3；覆盖旧「仅 turnSystemHint」 |
| U6 | 选中文段右键小菜单 | ✅ | ❌ CSS 部分 | §4 |
| U7 | 资源树文件夹展开记忆 | ✅ | ⏳ 文件已拷贝待验 | §5；`explorerExpandPrefs` |

建议顺序：**U7**（独立、Android 已有同构 FileTree）→ **U4** → **U5**（依赖 Composer + agentLoop）→ **U6**（可全局挂）。
...
[`OPEN-agent-ui-parity.md`](./OPEN-agent-ui-parity.md) §2.3「选 skill 写入 `/id `」与 §2.4「仅 turnSystemHint 催 read_skill」以**本文为准**（胶囊 + 正文注入）。
