...
## 176. Agent 对话按 Cursor 排版

去掉「你 / 代理人」角色标签。用户气泡改成中性圆角胶囊。工具结果改成 `>_` 终端块：左边动作、右边小写 `git`/`edit`，不再用蓝 GIT 徽章和「已执行」。助手不再展示 `⚙ git_add` 占位行。思考中是弱提示；调用工具时直接出现终端块。Ctrl+R 即可；`agentLoop` 改过，须**完整退出**后再测新对话。

## 177. Agent 顶栏去掉型号和用量数字

标题不再写模型名（底部选择器已有）。上下文条只留「上下文」和色条，具体 token / 百分比点开详情才显示。Ctrl+R 即可。

## 178. 作曲框回形针与发送同尺寸

回形针图标拉到与发送圆钮同直径（22px），箭头留在圆内。Ctrl+R 即可。

## 179. @ 引用文件菜单

输入 `@` 弹出工作区文件列表（最近标签优先，可继续输入过滤）。回车/点击把文件挂到作曲框芯片上，并清掉 `@…`。「文件和文件夹」在菜单里进入目录：点文件夹往下走，列表可滚动预览。Ctrl+R 即可。

## 180. 栏位分隔线统一细线

资源管理器/编辑器、编辑器/Agent 两条竖线都是同一根 1px `--border`，不再用 transform 细线（高 DPI 下会糊成两像素）。Ctrl+R 即可。

## 181. 中断对话 + 就地改最近一条问题

作曲框停止会留下半截回复（标「已停止」），Escape 也可停。悬停最后一条用户气泡可编辑后重发：半道截停或已经答完的最近一问都可以改，更早的历史不能改。改完会丢掉该问之后的助手/工具记录再跑一轮。须**完整退出** Electron 后再测。

## 182. 改问题并重发时撤回文件改动

重写最近一条问题若那一轮写过文件，先弹出确认（列出路径）。确定后把文件恢复成该轮开始前的内容（新建的则删除），再截断对话重发。取消则留在编辑态、不发送。须**完整退出**后再测。

## 183. 回退对话时清掉隐藏文件的文件夹黄点
