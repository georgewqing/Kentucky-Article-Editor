...
> **Android**：[`../../android/project-memory/BOARD.md`](../../android/project-memory/BOARD.md) **A3 ❌ 要移植**（实现重写；勿 import `win/`）
...
| ffmpeg | 发版/开发均用 `npm run ensure-ffmpeg` 落到 `win/resources/ffmpeg/ffmpeg.exe`（gitignore）；`dist*` 经 `extraResources` 打进安装包。**禁止** `ffmpeg-static`。缺则错误码 `FFMPEG_NOT_FOUND` → i18n Toast |
...
真源：`win/src/shared/kyboardSchema.ts`（主进程 re-export：`main/storyboard/kyboardSchema.ts`）。
...
- 已有可运行 ffmpeg → **复制**到 `win/resources/ffmpeg/ffmpeg.exe`
...
1. 解析 ffmpeg：见上一小节（`KENTUCKY_FFMPEG` → 打包 extraResources → 开发 `win/resources/ffmpeg` → 常见 Win 路径 → PATH）。
...
```
win/src/shared/kyboardSchema.ts
win/src/main/index.ts                         # kentucky-file 协议（mp3/mp4 流式 + Range）
win/src/main/storyboard/{pngUtil,storyboardService,registerStoryboardIpc,kyboardSchema}.ts
win/src/preload/index.ts                      # storyboard* + dialogs + toMediaUrl
win/src/renderer/index.html                   # CSP media-src kentucky-file:
win/src/renderer/src/platform/index.ts
win/src/renderer/src/editors/StoryboardEditor.tsx
win/src/renderer/src/editors/storyboardTimelineHelpers.ts
win/src/renderer/src/editors/ImagePreviewEditor.tsx
win/src/renderer/src/editors/VideoPreviewEditor.tsx
win/src/renderer/src/state/appStore.ts         # kind storyboard|image|video + createStoryboard
win/src/renderer/src/workbench/{EditorArea,FloatWorkbench,Sidebar,FileTree,explorerNames}
win/src/renderer/src/workbench/fitContextMenu.ts  # §116 右键贴边（工作台；非分镜专属）
win/src/renderer/src/styles/global.css         # .storyboard-* / .image-preview-* / .video-preview-* / scrub
win/src/renderer/src/styles/storyboard-nle.css # 时间线 NLE 壳
win/src/renderer/src/styles/storyboard-pages.css # 稿纸/导出/检视器抛光
win/src/renderer/src/main.tsx                  # 引入上述 CSS
win/src/renderer/src/i18n/locales/{en,zh-CN}.json  # storyboard.* / image.* / video.* / ffmpegNotFound
win/scripts/ensure-ffmpeg.js                   # 复制或 winget 安装 → resources/ffmpeg/ffmpeg.exe
win/resources/ffmpeg/README.txt                # 仓内占位；*.exe gitignore
...
- ffmpeg / 大图内存 / PNG 手势 / SAF 写 assets 按 [`PORTING-WIN-TO-ANDROID.md`](../../android/project-memory/PORTING-WIN-TO-ANDROID.md) §8.3 重写。
