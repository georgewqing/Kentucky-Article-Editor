...
- **Dialogue / 台词:** Chat-style `*.dialogue.csv` (11 cols incl. optional Godot staging `focus_node`/`font_size`/`text_color`) + root `characters.csv` (`model_node`); file meta `*.dialogue.meta.json`; auto filename from scene + dialogue id. Protocol **v1.1**: [`extras/godot-kentucky-dialogue/README.md`](./extras/godot-kentucky-dialogue/README.md). / 聊天式台词（含演出声明列）；meta + 节点名；Godot 契约 v1.1 见 extras。
