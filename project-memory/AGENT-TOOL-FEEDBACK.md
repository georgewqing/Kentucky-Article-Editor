...
| Win | Android |
|-----|---------|
| `src/main/ai/proposalGate.ts` | 安卓 `src/ai-runtime/proposalGate.ts` |
| `src/main/ai/tools.ts` | 安卓 `src/ai-runtime/tools.ts` |
| `src/main/ai/agentLoop.ts` | 安卓 `src/ai-runtime/agentLoop.ts` |
| `src/main/ai/planFiles.ts` | 安卓同协议 `plans/*.plan.md` |
| `src/main/ai/formats.ts` | 安卓 `src/ai-runtime/formats.ts` |
| `src/main/ai/webSearch.ts` | 安卓 `src/ai-runtime/webSearch.ts` |
| `src/renderer/.../proposalDiff.ts` | 安卓 AiPanel / diff |
| `src/renderer/.../AiPanel.tsx` | 安卓 AiPanel（窄宽 drawer） |
| `src/renderer/.../aiStore.ts` | 安卓 ai store / bridge |
...
> **Android 完整移植书**：独立工程 `project-memory/open/literary-memory.md`
...
| Agent UI（另 OPEN） | `/` skills+commands（可滑无滑块）；`contextEstimate` buckets；色条按 **limit**；冷灰蓝色板；消息区 `overflow-x:hidden` — 安卓：`open/agent-ui.md` |
...
9. **Round H**：见 [`AGENT-TOOL-TEST-BASELINE.md`](./AGENT-TOOL-TEST-BASELINE.md) §四；安卓详约见独立工程 `open/literary-memory.md`。
