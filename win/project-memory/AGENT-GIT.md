...
> **Android 契约**：[`../../android/project-memory/open/auto-apply-git.md`](../../android/project-memory/open/auto-apply-git.md)（**要移植**；isomorphic-git，禁止 `git.exe`）  
...
- 真源：`win/src/main/ai/workspacePath.ts`（`resolveWorkspacePath` / `assertInsideWorkspace` / `assertSafeExternalGitPath` / **`assertSafeWorkspaceRoot`**）+ `win/src/main/ipcSandbox.ts`（窗口绑定）。
...
| 路径 | 职责 |
|------|------|
| `win/src/main/ai/workspacePath.ts` | **工作区沙箱**（resolve/assert/safe bare/**safe workspace root**/dialog allowlist） |
| `win/src/main/ipcSandbox.ts` | 窗口工作区绑定、导航锁、`requireSenderWorkspace` |
| `win/src/main/git/gitService.ts` | probe / **configureGitExecutable** / ensure / status / diff / add / commit / remote* / pull / push / bare / **Git L5** / `formatGitCommitFailure` / `findWorkspaceGitEnvDoc`；**inspectWorkspaceGit / findGitRoot 只认 .git 目录** |
| `win/src/main/git/registerGitIpc.ts` | `git:*` IPC（绑定窗口工作区） |
