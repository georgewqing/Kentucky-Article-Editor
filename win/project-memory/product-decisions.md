...
| 开发优先级 | Win 为功能真源；**Android 移植 Win 已有全部产品功能**（壳不照搬）。源码分家，禁止整目录覆盖 |
| 安卓 | 独立软件根 `../android/`；大屏平板 + 优先外接键盘；不做手机布局；不移植 Electron 壳。功能清单与进度：[`../../android/project-memory/PORTING-WIN-TO-ANDROID.md`](../../android/project-memory/PORTING-WIN-TO-ANDROID.md) |
| **版本** | **v0.3.0**：分镜头稿本 + 简化 PR（无 AI）；v0.2.0 起内置文学向 AI 代理人 |
