...
**范围：** `Kentucky/win` Electron 桌面端（不含 Android、不含 `node_modules`）  
...
| 级别 | 位置 | 问题 | §121 |
|------|------|------|------|
| P0 | `win/src/main/index.ts` `fs:delete` / `fs:writeFile` / `fs:rename` / `fs:mkdir` / `fs:copyFile` / `doc:*` | 主进程 IPC **不校验工作区**。渲染层任意路径可读写下删；`fs:delete` 对目录 `rm({ recursive: true, force: true })` | 已修 |
| P0 | `win/src/main/index.ts` `createWindow` | **无** `will-navigate` / `setWindowOpenHandler`。若窗口导航到外站，preload 的 `window.kentucky` 仍在 | 已修 |
| P0 | `win/src/main/git/registerGitIpc.ts` `git:setPath`；打开工作区无系统路径拒绝 | `git:setPath` 可把任意 exe 当 git；用户若把盘符根 / `C:\Users\…` 当工作区打开，Agent `workspace_delete` 可清空其下全部内容 | 已修 |
| P1 | `win/src/main/index.ts` `kentucky-file` + `fs:toMediaUrl` | 协议按 query `path` **原样读任意本地文件**（含 Range 流 MP3/MP4） | 已修 |
| P1 | `win/src/renderer/src/editors/StoryboardEditor.tsx` + `platform.joinPath` | `.kyboard` 的 `imageRel`/`audioRel` 含 `..` 时拼出工作区外绝对路径 | 已修（renderer 消化 `..`；主进程仍拒） |
| P1 | `win/src/main/storyboard/storyboardService.ts` `exportMp4` | clip `duration` 无上限 → 巨量 PNG 帧 + 最长 600s ffmpeg，可把磁盘写满 | 已修（15 分钟） |
| P1 | `win/src/main/storyboard/pngUtil.ts` `decodePng`；`generateBlankSheet` / `sheetPixelSize` | PNG 无宽高上限；layout `cols`/`rows` 无上限 → 内存炸弹 / 卡死 | 已修 |
| P1 | `win/src/main/storyboard/storyboardService.ts` `importSheetFile` | `sourceAbs` 任意可读文件可被拷进工作区 | 已修（allowlist） |
| P1 | `win/src/main/git/gitService.ts` `gitInit` / `ensureWorkspaceGit` / `findGitRoot` | Git IPC 的 `workspaceRoot` 不跟窗口工作区绑定；`findGitRoot` 向上 40 层可能操作到**父仓** | 已修（不向上） |
| P2 | `win/src/main/ai/workspacePath.ts` `assertInsideWorkspace` | `realpath` 失败被吞掉，symlink/junction 检查可能跳过 | 已修（fail-closed） |
| P2 | `win/src/main/storyboard/storyboardService.ts` ffmpeg `filter_complex` | `fadeInSec` 等来自 JSON；`execFile` 无 shell，数值异常多为 `NaN` | 已修（`finiteNum`） |
| P2 | `win/src/main/ai/webSearch.ts` `fetchPageExcerpt` | 已限 `http(s)`；仍可打内网 URL（SSRF） | 已修（拒私网） |
| P2 | `win/src/renderer/index.html` CSP | `script-src` 含 `'unsafe-eval'`（Monaco 需要） | **保留** |
...
```31:35:win/src/main/git/registerGitIpc.ts
...
```188:197:win/src/renderer/src/platform/index.ts
...
`findGitRoot` 向上找 `.git`。打开 `Kentucky/win` 时可能命中容器仓 `Kentucky/`，Agent 的 commit/push 会碰到安卓树和其它工程。`git:init` 的根路径未与窗口工作区绑定。
...
实现入口：`win/src/main/ipcSandbox.ts`（窗口绑定、allowlist 断言、导航锁）+ `win/src/main/ai/workspacePath.ts`（路径几何、危险根、对话框 Set）。
...
| 打开 `Kentucky/win` | **不会**对容器仓 `Kentucky/` add/commit/push |
