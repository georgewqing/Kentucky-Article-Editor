...
3. **台词对话** — 节点图画布编辑 `.dialogue.csv`（11 列）+ `*.dialogue.choices.json`（播放图）+ `characters.csv`（含 `operable`）+ meta/layout；Godot 协议 **v1.3** 见 extras；执行器 [ai_river_godot](https://github.com/CCFOX12/ai_river_godot)
...
| [extras/godot-kentucky-dialogue](../extras/godot-kentucky-dialogue/README.md) | **Godot 接入协议 v1.3**（choices 播放图、`characters.operable`、空 text=确认/自动、节点声明器、11 列/meta；本仓不含插件） |
