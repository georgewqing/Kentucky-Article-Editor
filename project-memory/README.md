...
3. **台词对话** — `.dialogue.csv` + `characters.csv`（含 `model_node`）+ `*.dialogue.meta.json`；稳定 id；Godot 同目录热编辑协议见 extras  
...
| [extras/godot-kentucky-dialogue](../extras/godot-kentucky-dialogue/README.md) | **Godot 接入协议**（列/meta/`model_node`/文件名规则/监视建议；不含插件代码） |
